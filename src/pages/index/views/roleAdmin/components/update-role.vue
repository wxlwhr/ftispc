  ...
  <div class="wrapper">
    ...
    <el-form ref="ruleForm" :model="ruleForm" label-width="auto" status-icon :rules="rules">
      <el-form-item prop="role_bel_type" placeholder="请输入" label="角色所属:">
        <Slt :listParameter="roleBelType" class="select_box" :selectValue="ruleForm.role_bel_type" v-model="ruleForm.role_bel_type"></Slt>
      </el-form-item>
      ...
      <el-form-item  prop="role_des" label="角色描述:">
        <el-input type="textarea" v-model="ruleForm.role_des" clearable></el-input>
      </el-form-item>
      <el-form-item label="授权书模板:">
      ...
      <el-form-item prop="audit_type" label="审批模式:">
        <Slt :listParameter="auditType" class="select_box" :selectValue="ruleForm.audit_type" v-model="ruleForm.audit_type"></Slt>
      </el-form-item>
    </el-form>
...
<script>
import { updateOrgRole,getSelectData,downloadFile } from "@/api/api.js";
export default {
  name: "updateRole",
  components: {},
  data() {
  ...
  methods: {
    updateRole(){
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          ...
          data.append("role_id", this.ruleForm.role_id);
          data.append("auth_material_file", this.auth_material_file);
          updateOrgRole(data).then(function (response) {})
    ...
    handlexsyc(value){
      ...
      this.ruleForm.is_open = parseInt(this.ruleForm.is_open)
      this.fileList=[]
      if(this.ruleForm.source_filename!==''&&this.ruleForm.source_filename!=null){
        this.fileList.push({"name":this.ruleForm.source_filename,"path":this.ruleForm.target_path})
      }
    }
  },
};
