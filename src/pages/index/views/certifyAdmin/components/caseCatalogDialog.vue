...
<script>
import { insertCaseCatalogOne, insertCaseCatalogTow, updateCaseCatalogOne } from "@/api/api.js";
export default {
    props:{
        select:{
            type:String,
    ...
    methods: {
        ...
        submitChangeData(){
            this.$refs["addForm"].validate(valid => {
                if (valid) {
                    let insertOpenSourceCatalog = this.level == 1 ? insertCaseCatalogOne : insertCaseCatalogTow
                    insertOpenSourceCatalog(this.addForm).then((response)=> {
                        this.$message({
                            type: "success",
                            message: "添加成功!",
                        });
        ...
        editChangeData(){
            this.$refs["addForm"].validate(valid => {
                if (valid) {
                    ...
                    updateCaseCatalogOne(this.addForm).then((response)=> {
                        this.$message({
                            type: "success",
                            message: "修改成功!",
                        });
