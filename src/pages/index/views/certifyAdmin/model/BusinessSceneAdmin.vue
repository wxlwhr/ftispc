...
<template>
  <div class="wrapper">
    <el-container class="organ_menu">
      <el-header height="57px">业务细分场景管理</el-header>
      <el-container>
        ...
        <el-main>
          ...
          >
            ...
            <el-table-column prop="is_support_auth" label="是否支持认证"></el-table-column>
...
import { getSubCatalogList, getSubjecttree, deleteProductCatalog} from "@/api/api.js";
export default {
  data() {
    return {
  ...
  methods: {
    ...
    changeTreeData(e, val) {
      ...
      this.getSubCatalogLists(e.data.id)
    },
    // 刷新树
    menuRefresh(val) {
      this.getSubjecttree();
      this.getSubCatalogLists(val)
    },
    // 添加
    addTreeNode(e) {
      ...
      if(this.nodeData){
        ...
        if(level == 1){
          ...
          this.$refs.mysecond.parentHandleclick(this.parent_catalog);
        }else if(level == 3){
          this.$message({
              type: "error",
              message: "最多只能添加三级哦!",
    ...
    handleDelete(val) {
      ...
      this.$confirm("此操作将永久删除该文件, 是否继续?", "提示", {
        ...
        .then(() => {
          // 删除请求
          deleteProductCatalog({ catalog_id: val.catalog_id }).then(function (response) {
            ...
            if (response.code == "1") {
              ...
              that.getSubjecttree();
              that.getSubCatalogLists(val.parent_catalog)
            } else {
